...
Route::group(['middleware' => config('fortify.middleware', ['web'])], function () {
    ...
    Route::post(RoutePath::for('login', '/login'), [\App\Http\Controllers\AuthController::class, 'store'])
        ->middleware(array_filter([
            'guest:' . config('fortify.guard'),
            $limiter ? 'throttle:' . $limiter : null,
        ]));

    Route::post(RoutePath::for('logout', '/logout'), [\App\Http\Controllers\AuthController::class, 'logOut'])
        ->name('logout');
